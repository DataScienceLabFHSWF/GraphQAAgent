...
Our retrieval design is informed by recent literature (see `C3_LITERATURE.md` for
full synthesis):

- **Neural-symbolic integration** (Liu et al. 2024; Zhang et al. 2021) — symbolic
  graph traversal + neural embeddings fused via adaptive weighting. Surveys show
  hybrid approaches outperform either paradigm alone on real-world KGs.
- **Operator-centric design** (Yu 2025) — across 12 GraphRAG methods, retrieval
  operators (PPR, entity linking, RRF, reranking) are stronger predictors of
  quality than graph schema complexity. This validates our emphasis on the
  **retrieval pipeline** over graph enrichment.
- **Faithful reasoning** (Luo et al. 2025, GCR / ICML 2025) — integrating graph
  structure into generation, not just retrieval. Our post-hoc citation
  verification in the Explainer is a lightweight analogue of GCR's constrained
  decoding.
- **Adaptive architecture** (Yu 2025) — specific QA favours entity-centric + VDB
  operators; abstract QA favours community/subgraph operators. Our adaptive
  strategy selection and fusion weights address this directly.
...
## 9. Future Extensions (Literature-Inspired)

Based on the surveyed literature, the following extensions are planned or under
consideration:

### 9a. Constrained Decoding (from GCR, Luo et al. 2025)

Replace soft citation-checking in the Explainer with **hard constrained decoding**
where the LLM can only reference entities/relations present in the retrieved
subgraph. This would require building a KG-Trie index over the retrieved subgraph
at inference time.

### 9b. Personalized PageRank for Subgraph Ranking (from Yu 2025)

HippoRAG's success with PPR-based traversal suggests adding a PPR-based node
scoring step within the GraphRetriever. Instead of uniform k-hop expansion,
PPR would prioritise nodes most relevant to the query entity.

### 9c. Community-Based Retrieval for Abstract QA (from Yu 2025)

For high-level / abstract questions (e.g., "What are the main decommissioning
challenges?"), a community-based retrieval mode (inspired by LGraphRAG) could
summarise entity clusters from the KG — complementing our current entity-centric
and subgraph modes.

### 9d. Iterative Graph Exploration (from Think-on-Graph, Sun et al. 2023)

Extend the Orchestrator with an iterative loop: if the initial retrieval
confidence is low, let the LLM suggest next entities/relations to explore,
re-query the KG, and refine the answer. Validated by the Think-on-Graph framework
surveyed in Liu et al. 2024.
